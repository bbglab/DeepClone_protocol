...
def plot_samples_grid(df, output_prefix, gene2color=None):
    ...
    plt.tight_layout()
    fig.savefig(f"{output_prefix}.samples_grid.pdf", bbox_inches='tight', dpi=300)
    plt.close(fig)
    # Save results to TSV
...
def plot_genes_grid(df, output_prefix):
    ...
    plt.tight_layout()
    fig.savefig(f"{output_prefix}.genes_grid.pdf", bbox_inches='tight', dpi=300)
    plt.close(fig)
...
def main( output_prefix):
    ...
    syn_muts_df = pd.read_table("syn_muts.all_samples_compiled.tsv", sep = '\t', header = 0)
    ...
    syn_muts_df_samples_gene = syn_muts_df_samples[(syn_muts_df_samples["GENE"].isin(
        ["KMT2D", "NOTCH2", "STAG2", "TERTpromoter"])
        )]
    syn_muts_df_samples_selected = syn_muts_df_samples[(syn_muts_df_samples["SAMPLE"].isin(
        ["P19_0020_BTR_01", "P19_0023_BDO_01"])
        )]

    # # Per-gene and per-sample separated (PDF)
    # plot_per_gene_separated(syn_muts_df_samples_gene, output_prefix + ".samples")
    # plot_per_sample_separated(syn_muts_df_samples_selected, output_prefix + ".samples", gene2color=gene2color)

    plot_genes_grid(syn_muts_df_samples_gene, output_prefix + ".samples")
    plot_samples_grid(syn_muts_df_samples_selected, output_prefix + ".samples", gene2color=gene2color)
